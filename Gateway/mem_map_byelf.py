...
def get_memory_address(location):
    """Parse DWARF location expression to extract memory address."""
    try:
        if location[0] == 3:  # DW_OP_addr
            addr_bytes = location[1:5]  # Assuming 4-byte address
            address = sum(b << (i * 8) for i, b in enumerate(addr_bytes[::-1]))
